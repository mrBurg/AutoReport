...
Sub MoveRowsWithNotAllTrue()
    ...
    Set headerRange = wsSrc.Range("A1:P2")
    ...
    For i = startRow To lastRow
        If wsSrc.Cells(i, "F").Value = False Or wsSrc.Cells(i, "F").Value = "FALSE" Then
            GoTo SkipRow
        End If
    
        allTrue = True
        
        If wsSrc.Cells(i, 4).Value <> True And wsSrc.Cells(i, 4).Value <> "TRUE" Then
            allTrue = False
        Else
            allTrue = True
        End If
...
SkipRow:
    ...
    MsgBox "Перенос завершён. Перемещено " & destRow - startRow & " строк.", vbInformation
